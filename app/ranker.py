...
def _experience_score(user_years: int, exp_min: int) -> tuple[float, str]:
    ...
    # slight decay for heavily over-qualified candidates (>5y above min)
...
def rank_jobs(user: UserProfileModel, jobs: List[JobModel]) -> List[RankedJob]:
    results: List[RankedJob] = []

    for job in jobs:
        s_score, s_reason = _skills_score(user.skills or [], job.required_skills or [])
        r_score, r_reason = _role_score(user.preferred_roles or [], job.title)
        ...
        reasons = [s_reason, r_reason, e_reason, l_reason, p_reason]
